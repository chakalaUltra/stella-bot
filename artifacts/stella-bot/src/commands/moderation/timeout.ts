...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction, _client: StellaClient) {
    ...
    if (!duration) {
      return interaction.reply({ ...errReply("Invalid duration. Use: `10s`, `5m`, `2h`, `1d`"), ephemeral: true });
    }

    if (duration > 28 * 24 * 60 * 60 * 1000) {
      return interaction.reply({ ...errReply("Maximum timeout duration is 28 days."), ephemeral: true });
    }

    if (!target.moderatable) {
      return interaction.reply({ ...errReply("I cannot timeout this user."), ephemeral: true });
    }
